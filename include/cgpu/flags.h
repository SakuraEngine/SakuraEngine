...
{
    CGPU_RT_NONE = 0,
    CGPU_RT_SAMPLER = 0x01,
    // SRV Read only texture
    CGPU_RT_TEXTURE = (CGPU_RT_SAMPLER << 1),
    /// RTV Texture
    CGPU_RT_RENDER_TARGET = (CGPU_RT_TEXTURE << 1),
    /// DSV Texture
    CGPU_RT_DEPTH_STENCIL = (CGPU_RT_RENDER_TARGET << 1),
    /// UAV Texture
    CGPU_RT_RW_TEXTURE = (CGPU_RT_DEPTH_STENCIL << 1),
    // SRV Read only buffer
    CGPU_RT_BUFFER = (CGPU_RT_RW_TEXTURE << 1),
    CGPU_RT_BUFFER_RAW = (CGPU_RT_BUFFER | (CGPU_RT_BUFFER << 1)),
    /// UAV Buffer
    CGPU_RT_RW_BUFFER = (CGPU_RT_BUFFER << 2),
    CGPU_RT_RW_BUFFER_RAW = (CGPU_RT_RW_BUFFER | (CGPU_RT_RW_BUFFER << 1)),
    /// Uniform buffer
    CGPU_RT_UNIFORM_BUFFER = (CGPU_RT_RW_BUFFER << 2),
    /// Push constant / Root constant
    CGPU_RT_ROOT_CONSTANT = (CGPU_RT_UNIFORM_BUFFER << 1),
    /// IA
    CGPU_RT_VERTEX_BUFFER = (CGPU_RT_ROOT_CONSTANT << 1),
    CGPU_RT_INDEX_BUFFER = (CGPU_RT_VERTEX_BUFFER << 1),
    CGPU_RT_INDIRECT_BUFFER = (CGPU_RT_INDEX_BUFFER << 1),
    /// Cubemap SRV
    CGPU_RT_TEXTURE_CUBE = (CGPU_RT_TEXTURE | (CGPU_RT_INDIRECT_BUFFER << 1)),
    /// RTV / DSV per mip slice
    CGPU_RT_RENDER_TARGET_MIP_SLICES = (CGPU_RT_INDIRECT_BUFFER << 2),
    /// RTV / DSV per array slice
    CGPU_RT_RENDER_TARGET_ARRAY_SLICES = (CGPU_RT_RENDER_TARGET_MIP_SLICES << 1),
    /// RTV / DSV per depth slice
    CGPU_RT_RENDER_TARGET_DEPTH_SLICES = (CGPU_RT_RENDER_TARGET_ARRAY_SLICES << 1),
    CGPU_RT_RAY_TRACING = (CGPU_RT_RENDER_TARGET_DEPTH_SLICES << 1),
#if defined(CGPU_USE_VULKAN)
    /// Subpass input (descriptor type only available in Vulkan)
    CGPU_RT_INPUT_ATTACHMENT = (CGPU_RT_RAY_TRACING << 1),
    CGPU_RT_TEXEL_BUFFER = (CGPU_RT_INPUT_ATTACHMENT << 1),
    CGPU_RT_RW_TEXEL_BUFFER = (CGPU_RT_TEXEL_BUFFER << 1),
    CGPU_RT_COMBINED_IMAGE_SAMPLER = (CGPU_RT_RW_TEXEL_BUFFER << 1),
#endif
#if defined(CGPU_USE_METAL)
    CGPU_RT_ARGUMENT_BUFFER = (CGPU_RT_RAY_TRACING << 1),
    CGPU_RT_INDIRECT_COMMAND_BUFFER = (CGPU_RT_ARGUMENT_BUFFER << 1),
    CGPU_RT_RENDER_PIPELINE_STATE = (CGPU_RT_INDIRECT_COMMAND_BUFFER << 1),
#endif
    CGPU_RT_MAX_ENUM_BIT = 0x7FFFFFFF
} ECGPUResourceType;
