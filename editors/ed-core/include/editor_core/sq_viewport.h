...
class SKR_EDITOR_CORE_API SQViewport : public QOpenGLWidget
{
    Q_OBJECT
public:
    SQViewport(QWidget *partent) noexcept;
    ~SQViewport() noexcept;
...
private:
    ...
    ImportedTexture importTexture(uint64_t cgpu_handle, uint32_t memorySize, uint32_t mipLevels, int oglFormat, uint32_t width, uint32_t height);
