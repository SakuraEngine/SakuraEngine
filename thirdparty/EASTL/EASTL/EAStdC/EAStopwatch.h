...
#define EASTDC_LOCAL EASTL_EASTDC_LOCAL
...
namespace EA
{
	namespace StdC
	{
		...
		public:
			...
			enum Units
			{
				kUnitsCycles = 0, /// Stopwatch clock ticks. May or may not match CPU clock ticks 1:1, depending on your
				                  /// hardware and operating system. Some CPUs' low level cycle count instruction counts
				                  /// every 16 cycles instead of every cycle.
				kUnitsCPUCycles = 1, /// CPU clock ticks (or similar equivalent for the platform). Not recommended for
				                     /// use in shipping softare as many systems alter their CPU frequencies at runtime.
		...
		public:
			...
			explicit Stopwatch(int nUnits = kUnitsCycles, bool bStartImmediately = false)
			    : mnStartTime(0), mnTotalElapsedTime(0), mnUnits(nUnits), mfStopwatchCyclesToUnitsCoefficient(1.f)
			{
		...
		private:
			...
			float
			    mfStopwatchCyclesToUnitsCoefficient; /// Coefficient is defined seconds per cycle (assuming you want to
			                                         /// measure seconds). This is the inverse of the frequency, done
			                                         /// this way for speed. Time passed = cycle count * coefficient.
		...
		public:
			...
			LimitStopwatch(int nUnits = kUnitsCycles, uint64_t nLimit = 0, bool bStartImmediately = false)
			    : Stopwatch(nUnits, bStartImmediately), mnEndTime(nLimit)
			{
			}
...
inline bool EA::StdC::LimitStopwatch::IsTimeUp() const
{
	const uint64_t nCurrentTime = GetStopwatchCycle();
	return (int64_t)(mnEndTime - nCurrentTime) <
	       0; // We do this instead of a straight compare to deal with possible integer wraparound issues.
}


inline void EA::StdC::Stopwatch::Start()
{
...
inline void EA::StdC::Stopwatch::Stop()
{
...
inline void EA::StdC::Stopwatch::Restart()
{
	mnStartTime = 0;
...
inline int EA::StdC::Stopwatch::GetUnits() { return mnUnits; }


inline uint64_t EA::StdC::Stopwatch::GetElapsedTime() const
{
...
inline float EA::StdC::Stopwatch::GetElapsedTimeFloat() const
{
	uint64_t nFinalTotalElapsedTime64(mnTotalElapsedTime);
...
// and speed.
inline uint64_t EA::StdC::Stopwatch::GetStopwatchCycle() { return GetCPUCycle(); }
