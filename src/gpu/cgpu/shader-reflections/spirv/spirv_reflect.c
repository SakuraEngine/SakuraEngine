    ...
    size_t* res_size)
{
    ...
    *pp_res = NULL;
    if (*res_size > 0)
    {
        *pp_res = (uint32_t*)cgpu_calloc(*res_size, sizeof(**pp_res));
        if (IsNull(*pp_res))
        {
...
static SpvReflectResult ParseNodes(SpvReflectPrvParser* p_parser)
{
    ...
    p_parser->nodes = (SpvReflectPrvNode*)cgpu_calloc(p_parser->node_count, sizeof(*(p_parser->nodes)));
    if (IsNull(p_parser->nodes))
    {
        return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED;
    }
    ...
    {
        p_parser->access_chains = (SpvReflectPrvAccessChain*)cgpu_calloc(p_parser->access_chain_count, sizeof(*(p_parser->access_chains)));
        ...
        switch (p_node->op)
        {
            ...
            case SpvOpSource: {
                ...
                if (p_node->word_count >= 5)
                {
                    ...
                    char* p_source_temp = (char*)cgpu_calloc(source_len + 1, sizeof(char*));
            ...
            case SpvOpSourceContinued: {
                ...
                char* p_continued_source = (char*)cgpu_calloc(source_len + embedded_source_len + 1, sizeof(char*));
            ...
            case SpvOpAccessChain: {
                ...
                {
                    p_access_chain->indexes = (uint32_t*)cgpu_calloc(p_access_chain->index_count, sizeof(*(p_access_chain->indexes)));
...
static SpvReflectResult ParseStrings(SpvReflectPrvParser* p_parser)
{
    ...
    if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes))
    {
        ...
        p_parser->strings = (SpvReflectPrvString*)cgpu_calloc(p_parser->string_count, sizeof(*(p_parser->strings)));
...
static SpvReflectResult ParseSource(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module)
{
    ...
    if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code))
    {
        ...
        if (IsNotNull(p_parser->source_embedded))
        {
            ...
            char* p_source = (char*)cgpu_calloc(source_len + 1, sizeof(char*));
    ...
    size_t first_label_index)
{
    ...
    {
        p_func->callees = (uint32_t*)cgpu_calloc(p_func->callee_count,
    ...
    {
        p_func->accessed_ptrs = (uint32_t*)cgpu_calloc(p_func->accessed_ptr_count,
...
static SpvReflectResult ParseFunctions(SpvReflectPrvParser* p_parser)
{
    ...
    if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes))
    {
        ...
        p_parser->functions = (SpvReflectPrvFunction*)cgpu_calloc(p_parser->function_count,
        ...
        for (size_t i = 0; i < p_parser->function_count; ++i)
        {
            ...
            p_func->callee_ptrs = (SpvReflectPrvFunction**)cgpu_calloc(p_func->callee_count,
...
static SpvReflectResult ParseMemberCounts(SpvReflectPrvParser* p_parser)
{
    ...
    if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes))
    {
        ...
        for (uint32_t i = 0; i < p_parser->node_count; ++i)
        {
            ...
            p_node->member_names = (const char**)cgpu_calloc(p_node->member_count, sizeof(*(p_node->member_names)));
            if (IsNull(p_node->member_names))
            {
                return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED;
            }

            p_node->member_decorations = (SpvReflectPrvDecorations*)cgpu_calloc(p_node->member_count, sizeof(*(p_node->member_decorations)));
    ...
    uint32_t** pp_uniforms)
{
    ...
    *pp_uniforms = (uint32_t*)cgpu_calloc(*p_uniform_count, sizeof(**pp_uniforms));
    ...
    SpvReflectTypeDescription* p_type)
{
    ...
    if (p_node->member_count > 0)
    {
        ...
        p_type->members = (SpvReflectTypeDescription*)cgpu_calloc(p_type->member_count, sizeof(*(p_type->members)));
    ...
    SpvReflectShaderModule* p_module)
{
    ...
    p_module->_internal->type_descriptions = (SpvReflectTypeDescription*)cgpu_calloc(p_module->_internal->type_description_count,
    ...
    SpvReflectShaderModule* p_module)
{
    ...
    p_module->descriptor_bindings = (SpvReflectDescriptorBinding*)cgpu_calloc(p_module->descriptor_binding_count, sizeof(*(p_module->descriptor_bindings)));
    ...
    SpvReflectBlockVariable* p_var)
{
    ...
    if (IsNotNull(p_type->members) && (p_type->member_count > 0))
    {
        ...
        p_var->members = (SpvReflectBlockVariable*)cgpu_calloc(p_var->member_count, sizeof(*p_var->members));
    ...
    bool* p_has_built_in)
{
    ...
    {
        p_var->member_count = p_type->member_count;
        p_var->members = (SpvReflectInterfaceVariable*)cgpu_calloc(p_var->member_count, sizeof(*p_var->members));
        if (IsNull(p_var->members))
        {
            return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED;
        }
    ...
    uint32_t* p_interface_variable_ids)
{
    ...
    {
        p_entry->input_variables = (SpvReflectInterfaceVariable**)cgpu_calloc(p_entry->input_variable_count, sizeof(*(p_entry->input_variables)));
    ...
    {
        p_entry->output_variables = (SpvReflectInterfaceVariable**)cgpu_calloc(p_entry->output_variable_count, sizeof(*(p_entry->output_variables)));
    ...
    {
        p_entry->interface_variables = (SpvReflectInterfaceVariable*)cgpu_calloc(p_entry->interface_variable_count, sizeof(*(p_entry->interface_variables)));
    ...
    uint32_t** p_push_constants)
{
    ...
    *p_push_constants = (uint32_t*)cgpu_calloc(*p_push_constant_count, sizeof(**p_push_constants));
    ...
    uint32_t* push_constants)
{
    ...
    {
        p_called_functions = (uint32_t*)cgpu_calloc(called_function_count, sizeof(*p_called_functions));
    ...
    {
        used_variables = (uint32_t*)cgpu_calloc(used_variable_count,
    ...
    SpvReflectShaderModule* p_module)
{
    ...
    p_module->entry_points = (SpvReflectEntryPoint*)cgpu_calloc(p_module->entry_point_count,
    ...
    for (size_t i = 0; entry_point_index < p_parser->entry_point_count && i < p_parser->node_count; ++i)
    {
        ...
        if (interface_variable_count > 0)
        {
            p_interface_variables = (uint32_t*)cgpu_calloc(interface_variable_count, sizeof(*(p_interface_variables)));
    ...
    SpvReflectShaderModule* p_module)
{
    ...
    p_module->push_constant_blocks = (SpvReflectBlockVariable*)cgpu_calloc(p_module->push_constant_block_count, sizeof(*p_module->push_constant_blocks));
...
static SpvReflectResult ParseEntrypointDescriptorSets(SpvReflectShaderModule* p_module)
{
    ...
    for (uint32_t i = 0; i < p_module->entry_point_count; ++i)
    {
        ...
        {
            p_entry->descriptor_sets = (SpvReflectDescriptorSet*)cgpu_calloc(p_entry->descriptor_set_count,
...
static SpvReflectResult ParseDescriptorSets(SpvReflectShaderModule* p_module)
{
    ...
    for (uint32_t i = 0; i < p_module->descriptor_set_count; ++i)
    {
        ...
        p_set->bindings = (SpvReflectDescriptorBinding**)cgpu_calloc(p_set->binding_count, sizeof(*(p_set->bindings)));
    ...
    SpvReflectShaderModule* p_module)
{
...
#ifdef __cplusplus
    p_module->_internal = (SpvReflectShaderModule::Internal*)cgpu_calloc(1, sizeof(*(p_module->_internal)));
#else
    p_module->_internal = cgpu_calloc(1, sizeof(*(p_module->_internal)));
#endif
    ...
    p_module->_internal->spirv_code = (uint32_t*)cgpu_calloc(1, p_module->_internal->spirv_size);
    ...
    SpvReflectResult* p_result)
{
    ...
    {
        *p_result = IsNotNull(p_descriptor) ? SPV_REFLECT_RESULT_SUCCESS : (IsNull(p_module) ? SPV_REFLECT_RESULT_ERROR_NULL_POINTER : SPV_REFLECT_RESULT_ERROR_ELEMENT_NOT_FOUND);
    }
    return p_descriptor;
}
    ...
    SpvReflectResult* p_result)
{
    ...
    if (IsNotNull(p_result))
    {
        *p_result = IsNotNull(p_set) ? SPV_REFLECT_RESULT_SUCCESS : (IsNull(p_module) ? SPV_REFLECT_RESULT_ERROR_NULL_POINTER : SPV_REFLECT_RESULT_ERROR_ELEMENT_NOT_FOUND);
    }
    return p_set;
}
    ...
    SpvReflectResult* p_result)
{
    ...
    if (IsNotNull(p_result))
    {
        *p_result = IsNotNull(p_var) ? SPV_REFLECT_RESULT_SUCCESS : (IsNull(p_module) ? SPV_REFLECT_RESULT_ERROR_NULL_POINTER : SPV_REFLECT_RESULT_ERROR_ELEMENT_NOT_FOUND);
    }
    return p_var;
}
    ...
    SpvReflectResult* p_result)
{
    ...
    if (IsNotNull(p_result))
    {
        *p_result = IsNotNull(p_push_constant) ? SPV_REFLECT_RESULT_SUCCESS : (IsNull(p_module) ? SPV_REFLECT_RESULT_ERROR_NULL_POINTER : SPV_REFLECT_RESULT_ERROR_ELEMENT_NOT_FOUND);
    }
