...
#include "input/gainput/GainputInputManager.h"
...
namespace gainput {
        ...
        void Update(InputDeltaState* delta)
        {
            ...
            for (int i = 0; i < QUEST_REMOTE_MAX; ++i)
            {
                if (inputDevices_[i].capabilities.Header.DeviceID != ovrDeviceIdType_Invalid)
                {
                    ...
                    {
                        SKR_LOG_ERROR("Failed to get input state from device ID(%u)", inputDevices_[i].capabilities.Header.DeviceID);
                        continue;
                    }
    ...
    private:
        ...
        void OnDeviceConnected(ovrInputCapabilityHeader capHeader)
        {
            ...
            {
                SKR_LOG_INFO("Controller connected ID(%u)", capHeader.DeviceID);
                ...
                SKR_LOG_ERROR("Failed to connect controller ID(%u)", capHeader.DeviceID);
            }
        }
