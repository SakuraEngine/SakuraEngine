...
#if defined(_WINDOWS)
    SKR_ASSERT(gNotifier && "gNotifierInitialized should have guaranteed this was not null.");
    UnregisterDeviceNotification(gNotifier);
    gNotifier = NULL;
#elif defined(__APPLE__)
    SKR_ASSERT(gNotificationPort && "gNotifierInitialized should have guaranteed this was not null.");
    IONotificationPortDestroy(gNotificationPort);
    gNotificationPort = NULL;
#elif defined(__linux__)
    SKR_ASSERT(pUDev && pUDevMonitor && "gNotifierInitialized should have guaranteed these were not null.");
...
int HIDInit(void* window)
{
    ...
    // Init controller hidapi parsers
...
static void HIDInitControllerTracking()
{
    memset(gControllerBuffer, 0, sizeof(gControllerBuffer));
}
...
void HIDLoadController(uint8_t devID, uint8_t playerNum)
{
    SKR_ASSERT(devID > 0 && devID <= MAX_DEVICES_TRACKED);
    uint8_t index = devID - 1;

    if (!gDeviceBuffer[index].active)
    {
        SKR_LOG_WARN("Tried to load non-existant device at index %i", index);
        return;
    }
...
void HIDUnloadController(uint8_t devID)
{
    ...
    if (!con->hidDev)
    {
        SKR_LOG_WARN("Tried to unload non-existant controller at index %i", index);
        return;
    }

    SKR_ASSERT(con->Close && "All controllers must set a close method.");
