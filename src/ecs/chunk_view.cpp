...
namespace dual
{
    ...
    static void construct_impl(const dual_chunk_view_t& view, type_index_t type, EIndex offset
        , uint32_t size, uint32_t align, uint32_t elemSize, uint32_t maskValue, void (*constructor)(dual_chunk_t* chunk, EIndex index, char* data))
    {
    ...
    static void destruct_impl(const dual_chunk_view_t& view, type_index_t type, EIndex offset, uint32_t size
        , uint32_t elemSize, void (*destructor)(dual_chunk_t* chunk, EIndex index, char* data))
    {
        char* src = view.chunk->data() + (size_t)offset + (size_t)size * view.start;
        if(type.is_buffer())
            forloop(j, 0, view.count)
    ...
    static void move_impl(const dual_chunk_view_t& dstV, const dual_chunk_t* srcC, uint32_t srcStart, type_index_t type
        , EIndex offset, uint32_t size, uint32_t align, uint32_t elemSize, void (*move)(dual_chunk_t* chunk, EIndex index, char* dst, dual_chunk_t* schunk, EIndex sindex, char* src))
    {
    ...
    static void duplicate_impl(const dual_chunk_view_t& dstV, const dual_chunk_t* srcC, uint32_t srcIndex, type_index_t type
    , EIndex offset, EIndex dstOffset, uint32_t size, uint32_t elemSize, void (*copy)(dual_chunk_t* chunk, EIndex index, char* dst, dual_chunk_t* schunk, EIndex sindex, const char* src))
    {
    ...
    void cast_view(const dual_chunk_view_t& dstV, dual_chunk_t* srcC, EIndex srcStart) noexcept
    {
        ...
        while(srcI < srcTypes.length && dstI < dstTypes.length)
        {
            ...
            {
                destruct_impl({srcC, srcStart, dstV.count}, srcT, srcOffsets[srcI]
                    , srcSizes[srcI], srcElemSizes[srcI], srcType->callbacks[srcI].destructor);
                ++srcI;
            }
            ...
            else
            {
                if(srcT != kMaskComponent)
                    move_impl(dstV, srcC, srcStart, srcT, srcOffsets[srcI], srcSizes[srcI]
                        , srcAligns[srcI], srcElemSizes[srcI], srcType->callbacks[srcI].move);
                if(dstMasks)
                {
                    if(srcMasks)
                        forloop(i, 0, dstV.count)
                            dstMasks[i].set(dstI, srcMasks[i].test(srcI));
                    else
                        forloop(i, 0, dstV.count)
                            dstMasks[i].set(dstI);
                }
