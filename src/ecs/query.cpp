...
dual_query_t* dual_storage_t::make_query(const char* inDesc)
{
    ...
    for(auto part : parts)
    {
        ...
        operation.phase = -1;
        enum { OPTIONAL, ALL, ANY, NONE } selector = ALL;
        ...
        {
            auto j = i;
            while(i<part.size() && std::isalpha(part[i])) ++i;
        ...
        if(shared)
        {
            switch (selector) {
                case ALL:
                    all_shared.push_back(type); break;
                case ANY:
                    any_shared.push_back(type); break;
                case NONE:
                    none_shared.push_back(type); break;
                default: break; //optional wont be filtered
            }
        }
        else 
        {
            switch (selector) {
                case ALL:
                    all.push_back(type); break;
                case ANY:
                    any.push_back(type); break;
                case NONE:
                    none.push_back(type); break;
                default: break; //optional wont be filtered
            }
        }
    ...
    auto result = arena.allocate<dual_query_t>();
    #define FILTER_PART(NAME) std::sort(NAME.begin(), NAME.end()); result->filter.NAME = dual_type_set_t{NAME.data(), (SIndex)NAME.size()};
    FILTER_PART(all);
    FILTER_PART(any);
    FILTER_PART(none);
    ...
    else
    {
        ...
        if(nonemask == 0 && anymask == 0) // fastpath
        {
            ...
            while(c != nullptr)
            {
                ...
                while(i < count)
                {
                    while(i < count && !((masks[i] & allmask) == allmask) && i % 4 != 0) ++i;
                    if(i%4 == 0)
                    {
                        while(i < count - 4)
                        {
                        ...
                        if(i>=count - 4)
                            while(i < count && !((masks[i] & allmask) == allmask)) ++i;
                    }
                    view.start = i;
