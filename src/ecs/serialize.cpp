...
static void serialize_impl(const dual_chunk_view_t& view, dual_type_index_t type, EIndex offset, uint32_t size
    , uint32_t elemSize, dual::serializer_t s,  void (*serialize)(dual_chunk_t* chunk, EIndex index, char* data, EIndex count, const dual_serializer_v* v, void* s))
{
            ...
            forloop(i, 0, view.count)
            {
                ...
                *(intptr_t*)temp.EndX = ((char*)temp.EndX - (char*)temp.BeginX); //length
                ...
                if(serialize)
                    ...
                    s.archive(array->BeginX, ((char*)array->EndX - (char*)array->BeginX));
            }
        }
        else
        {
...
void dual_storage_t::serialize(dual::serializer_t s)
{
    ...
    for(auto& pair : groups)
    {
        ...
        s.archive((uint32_t)group->chunkCount);
