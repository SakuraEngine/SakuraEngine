...
typedef struct sugoi_callback_v
{
    ...
    // serialize
    bool (*serialize)(sugoi_type_index_t type, const void* data, EIndex count, skr::ArchiveWrite* reader) SKR_IF_CPP(= nullptr);
    bool (*deserialize)(sugoi_type_index_t type, void* data, EIndex count, skr::ArchiveRead* reader) SKR_IF_CPP(= nullptr);
...
/*** serialize API ***/
/*********************/
SKR_RUNTIME_API void sugoiS_serialize(sugoi_storage_t* storage, skr::ArchiveWrite* w);
SKR_RUNTIME_API void sugoiS_deserialize(sugoi_storage_t* storage, skr::ArchiveRead* r);
...
template <typename T, typename R = T>
skr::Span<R> get_components(sugoi_chunk_view_t* view)
{
    if (auto ptrs = get_owned<T, R>(view))
        return skr::Span<R>(ptrs, view->count);
    return {};
}
