...
TypeRegistry::Impl::Impl()
{
    ...
    }
    {
        ...
        desc.arrElementSize = sizeof(sugoi_entity_t);
    ...
    if (i != guid2type.end())
    {
        ...
        bool capable = true;
        {
            ...
            if (oldDesc.alignment != desc.alignment)
                capable = false;
            if (oldDesc.arrElementSize != desc.arrElementSize)
                capable = false;
...
void sugoi_make_guid(skr::GUID* guid)
{
...
{
    return sugoi::ArrayComponentBase::calc_inline_memory_size(
        elementSize,
        elementAlign,
        count
    );
