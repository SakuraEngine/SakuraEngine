...
[[callop("f32tof16")]] uint f32tof16(float value);
[[callop("f32tof16")]] uint2 f32tof16(float2 value);
[[callop("f32tof16")]] uint3 f32tof16(float3 value);
[[callop("f32tof16")]] uint4 f32tof16(float4 value);

[[callop("f16tof32")]] float f16tof32(uint value);
[[callop("f16tof32")]] float2 f16tof32(uint2 value);
[[callop("f16tof32")]] float3 f16tof32(uint3 value);
[[callop("f16tof32")]] float4 f16tof32(uint4 value);
...
template<concepts::arithmetic T, concepts::arithmetic U>
[[callop("MAX")]] extern T max(const T& v, const U& b) requires(vec_dim_v<T> == vec_dim_v<U>);
...
[[callop("ISINF")]] extern vec<bool, vec_dim_v<T>> isinf(const T& v);

template<concepts::float_vec_family T>
[[callop("ISNAN")]] extern vec<bool, vec_dim_v<T>> isnan(const T& v);

[[callop("ISINF")]] extern bool isinf(float v);

[[callop("ISNAN")]] extern bool isnan(float v);
...
template<concepts::float_vec_family T, concepts::float_family B> requires(same_dim_v<T, B> || is_scalar_v<B>)
auto lerp(const T& left_v, float right_v, const B& step) { return lerp(left_v, T(right_v), step); }
template<concepts::float_vec_family T, concepts::float_family B> requires(same_dim_v<T, B> || is_scalar_v<B>)
auto lerp(float left_v, const T& right_v, const B& step) { return lerp(T(left_v), right_v, step); }
template<concepts::float_vec_family V> 
auto lerp(float a, float b, const V& step) { return lerp(V(a), V(b), step); } 

template<concepts::arithmetic T, concepts::arithmetic_scalar U> requires(vec_dim_v<T> != vec_dim_v<U>)
auto min(const T& a, U b) { return min(a, T(b)); }
template<concepts::arithmetic T, concepts::arithmetic_scalar U> requires(vec_dim_v<T> != vec_dim_v<U>)
auto min(U a, const T& b) { return min(T(a), b); }

template<concepts::arithmetic T, concepts::arithmetic_scalar U> requires(vec_dim_v<T> != vec_dim_v<U>)
auto max(const T& a, U b) { return max(a, T(b)); }
template<concepts::arithmetic T, concepts::arithmetic_scalar U> requires(vec_dim_v<T> != vec_dim_v<U>)
auto max(U a, const T& b) { return max(T(a), b); }

template<concepts::arithmetic T, concepts::arithmetic_scalar U> requires(vec_dim_v<T> != vec_dim_v<U>)
auto dot(const T& a, U b) { return dot(a, T(b)); }
template<concepts::arithmetic T, concepts::arithmetic_scalar U> requires(vec_dim_v<T> != vec_dim_v<U>)
auto dot(U a, const T& b) { return dot(T(a), b); }

template<concepts::arithmetic_scalar T, concepts::arithmetic_vec V>
auto step(const T& a, const V& b) { return step(V(a), b); }