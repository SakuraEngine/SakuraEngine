...
{
public:
    const auto& columns() const { return _x; }
    const auto& rows() const { return _y; }
    const auto& element_type() const { return _element; }

protected:
    friend struct AST;
    MatrixTypeDecl(AST& ast, const TypeDecl* element, uint32_t x, uint32_t y, uint32_t alignment);
    const TypeDecl* _element = nullptr; 
    uint32_t _x = 0;
    uint32_t _y = 0;
};
