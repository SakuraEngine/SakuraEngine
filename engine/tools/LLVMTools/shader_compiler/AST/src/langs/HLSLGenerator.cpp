...
void HLSLGenerator::VisitConditionalExpr(SourceBuilderNew& sb, const ConditionalExpr* condExpr)
{
    if (condExpr->type()->is_vector() || condExpr->type()->is_matrix())
    {
        sb.append(L"select(");
    ...
    {
        CppLikeShaderGenerator::VisitConditionalExpr(sb, condExpr);
    }
}
...
void HLSLGenerator::VisitShaderResource(SourceBuilderNew& sb, const skr::CppSL::VarDecl* var)
{
    ...
    if (const auto asGloballyCoherent = FindAttr<GloballyCoherentAttr>(var->attrs()))
    {
        typeName = L"globallycoherent " + typeName;
    }

    if (const auto asNormView = FindAttr<NormFloatAccessAttr>(var->attrs()))
    {
        // regex replace '(RW)Texture2D<' to '(RW)Texture2D<snorm/unorm '
        const wchar_t* kNormToken = asNormView->is_signed() ? L"snorm " : L"unorm ";

        // 若已经带有 <unorm 或 <snorm，统一替换为期望 token
        {
            static const std::wregex kHasUnorm(L"<\\s*unorm\\s+");
            static const std::wregex kHasSnorm(L"<\\s*snorm\\s+");
            typeName = std::regex_replace(typeName, kHasUnorm, std::wstring(L"<") + kNormToken);
            typeName = std::regex_replace(typeName, kHasSnorm, std::wstring(L"<") + kNormToken);
        }

        // 若仍未包含规范化访问前缀，则插入到纹理泛型起始处
        if (typeName.find(L"<unorm ") == String::npos && typeName.find(L"<snorm ") == String::npos)
        {
            static const std::wregex kRwTex2D(L"\\bRWTexture2D\\s*<\\s*");
            static const std::wregex kTex2D(L"\\bTexture2D\\s*<\\s*");
            typeName = std::regex_replace(typeName, kRwTex2D, std::wstring(L"RWTexture2D<") + kNormToken);
            typeName = std::regex_replace(typeName, kTex2D, std::wstring(L"Texture2D<") + kNormToken);
        }
    }
...
void HLSLGenerator::VisitConstructExpr(SourceBuilderNew& sb, const ConstructExpr* ctorExpr)
{
    ...
    else
    {
        ...
        const bool is_matrix_cast = ctorExpr->type()->is_matrix() &&
            (ctorExpr->args().size() == 1) &&
            (ctorExpr->args()[0]->type()->is_matrix()) &&
            (ctorExpr->type() != ctorExpr->args()[0]->type());

        if (is_matrix_cast)
            sb.append(L"(" + GetQualifiedTypeName(ctorExpr->type()) + L")(");
        else if (ctorExpr->type()->is_builtin())
