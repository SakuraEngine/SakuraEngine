...
#define ReturnVecWithSameDim(T) \
    ...
    _intrinsics["Texture::Load"] = DeclareTemplateFunction(L"CppSLTextureLoad", 
        [=](auto pts) { 
    ...
    _intrinsics["Texture::Store"] = DeclareTemplateFunction(L"CppSLTextureStore", VoidType, TextureWriteParams);
    std::array<VarConceptDecl*, 1> TextureSizeParams = { TextureFamily };
    _intrinsics["Texture::Size"] = DeclareTemplateFunction(L"CppSLTextureSize", UInt3Type, TextureSizeParams);

    std::array<VarConceptDecl*, 3> SampleParams = { TextureFamily, SamplerFamily, FloatFamily };
    _intrinsics["Texture::Sample"] = DeclareTemplateFunction(L"CppSLTextureSample", [this](auto pts) {
    ...
    _intrinsics["Texture::SampleLevel"] = DeclareTemplateFunction(L"CppSLTextureSampleLevel", [this](auto pts) {
        auto pt = &dynamic_cast<const TextureTypeDecl*>(pts[1])->element_type();
        if (pt == FloatType) return Float4Type;
        if (pt == HalfType) return Half4Type;
        if (pt == IntType) return Int4Type;
