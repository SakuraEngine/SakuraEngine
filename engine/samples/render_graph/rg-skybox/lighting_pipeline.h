...
inline CGPUComputePipelineId create_lighting_compute_pipeline(CGPUDeviceId device)
{
    uint32_t *cs_bytes, cs_length;
    read_shader_bytes(SKR_UTF8("rg-skybox/lighting_cs.cs"), &cs_bytes, &cs_length, device->adapter->instance->backend);
...
inline CGPURenderPipelineId create_lighting_render_pipeline(CGPUDeviceId device, CGPUSamplerId static_sampler, ECGPUFormat format)
{
    uint32_t *vs_bytes, vs_length;
    uint32_t *fs_bytes, fs_length;
    read_shader_bytes(SKR_UTF8("rg-skybox/screen_vs.vs"), &vs_bytes, &vs_length, device->adapter->instance->backend);
    read_shader_bytes(SKR_UTF8("rg-skybox/lighting_fs.fs"), &fs_bytes, &fs_length, device->adapter->instance->backend);
