...
#endif // #if !defined(IMGUI_DISABLE_INCLUDE_IMCONFIG_H)|| defined(IMGUI_INCLUDE_IMCONFIG_H)
...
CIMGUI_API void        ImGui_ShowUserGuide(void);                             // add basic help/info block (not a window): how to manipulate ImGui as a end-user (mouse/keyboard controls).
...
CIMGUI_API ImVec2 ImGui_GetWindowContentRegionMax(void);  // content boundaries max for the full window (roughly (0,0)+Size-Scroll) where Size can be override with SetNextWindowContentSize(), in window coordinates
...
CIMGUI_API void   ImGui_NewLine(void);                            // undo a SameLine() or force a new line when in an horizontal-layout context.
...
CIMGUI_API void  ImGui_TreePushPtr(const void* ptr_id /* = NULL */);                                                               // "
...
CIMGUI_API ImGuiID ImGui_DockSpace(ImGuiID id);                                           // Implied size = ImVec2(0, 0), flags = 0, window_class = NULL
CIMGUI_API ImGuiID ImGui_DockSpaceEx(ImGuiID id, ImVec2 size /* = ImVec2(0, 0) */, ImGuiDockNodeFlags flags /* = 0 */, const ImGuiWindowClass* window_class /* = NULL */);
CIMGUI_API ImGuiID ImGui_DockSpaceOverViewport(void);                                     // Implied viewport = NULL, flags = 0, window_class = NULL
...
CIMGUI_API bool   ImGui_IsItemClickedEx(ImGuiMouseButton mouse_button /* = 0 */);  // is the last item hovered and mouse clicked on? (**)  == IsMouseClicked(mouse_button) && IsItemHovered()Important. (**) this it NOT equivalent to the behavior of e.g. Button(). Read comments in function definition.
...
CIMGUI_API bool   ImGui_IsItemDeactivated(void);                                   // was the last item just made inactive (item was previously active). Useful for Undo/Redo patterns with widgets that requires continuous editing.
CIMGUI_API bool   ImGui_IsItemDeactivatedAfterEdit(void);                          // was the last item just made inactive and made a value change when it was active? (e.g. Slider/Drag moved). Useful for Undo/Redo patterns with widgets that requires continuous editing. Note that you may get false positives (some widgets such as Combo()/ListBox()/Selectable() will return true even when clicking an already selected item).
...
CIMGUI_API ImGuiMouseCursor ImGui_GetMouseCursor(void);                                                          // get desired cursor type, reset in ImGui::NewFrame(), this is updated during the frame. valid before Render(). If you use software rendering by setting io.MouseDrawCursor ImGui will render those for you
CIMGUI_API void             ImGui_SetMouseCursor(ImGuiMouseCursor cursor_type);                                  // set desired cursor type
...
CIMGUI_API void ImGuiIO_AddInputCharacterUTF16(ImGuiIO* self, ImWchar16 c);                                           // Queue a new character input from an UTF-16 character, it can be a surrogate
CIMGUI_API void ImGuiIO_AddInputCharactersUTF8(ImGuiIO* self, const char* str);                                       // Queue a new characters input from an UTF-8 string
...
CIMGUI_API bool                   ImFontAtlas_IsBuilt(const ImFontAtlas* self);                                                                                      // Bit ambiguous: used to detect when user didn't built texture but effectively we should check TexID != 0 except that would be backend dependent...
...
#ifndef IMGUI_DISABLE_OBSOLETE_KEYIO
CIMGUI_API int GetKeyIndex(ImGuiKey key);  // map ImGuiKey_* values into legacy native key index. == io.KeyMap[key]
#else
CIMGUI_API int GetKeyIndex(ImGuiKey key);
#endif // #ifndef IMGUI_DISABLE_OBSOLETE_KEYIO
...
CIMGUI_API void  ImGui_OpenPopupContextItem(const char* str_id /* = NULL */, ImGuiMouseButton mb /* = 1 */);                             // Bool return value removed. Use IsWindowAppearing() in BeginPopup() instead. Renamed in 1.77, renamed back in 1.79. Sorry!
