...
-- run target
function _on_test_target(target)
...
function _test(target)
    ...
    ,   target:script("run", _on_test_target)
    ,   function (target)
            for _, r in ipairs(target:orderules()) do
...
function main()
    ...
    -- run the given target?
    if targetname then
        _test(project.target(targetname))
    else
        local targets = {}
        for _, target in ipairs(project.ordertargets()) do
            if target:is_binary() or target:script("run") then
        ...
        end, {total = #targets, comax = jobs, isolate = true})
    end
