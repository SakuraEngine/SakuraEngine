...
inline static void env_create(MDB_env** penv)
{
    ...
    {
        SKR_LOG_ERROR(u8"mdb_env_create failed: %d", rc);
    }
    ...
    {
        SKR_LOG_ERROR(u8"mdb_env_open failed: %d", rc);
    }
    else
    {
        SKR_LOG_INFO(u8"mdb_env_open succeed: %d", rc);
    }
}
...
inline static void dbi_create(MDB_env* env, MDB_dbi* pdb, bool readonly)
{
    ...
    {
        SKR_LOG_INFO(u8"mdb_txn_begin succeed: %d", rc);
    }
    // Txn body: open db
    {
        ...
        {
            SKR_LOG_ERROR(u8"mdb_dbi_open failed: %d", rc);
        }
        else if (!readonly)
        {
            if (const int rc = mdb_drop(txn, *pdb, 0) )
            {
                SKR_LOG_ERROR(u8"mdb_dbi_drop failed: %d", rc);
            }
        }
    }
    mdb_txn_commit(txn);
