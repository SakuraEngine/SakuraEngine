...
int main(int argc, char* argv[])
{
    ...
    auto view = smath::look_at_matrix({ 0.f, 50.f, 175.f } /*eye*/, { 0.f, 50.f, 0.f } /*at*/);
    ...
    while (!quit)
    {
        ...
        {
            if (SDL_GetWindowID(render_device->sdl_window_) == event.window.windowID)
            {
                quit = !SDLEventHandler(&event, render_device->sdl_window_);
            }
        }
        CGpuAcquireNextDescriptor acquire_desc = {};
        acquire_desc.signal_semaphore = present_semaphore;
        backbuffer_index = render_device->AcquireNextFrame(acquire_desc);
        const CGpuTextureId back_buffer = render_device->swapchain_->back_buffers[backbuffer_index];
        render_context->Wait();
        render_context->Begin();
        // render pass
        CGpuColorAttachment screen_attachment = {};
        screen_attachment.view = render_device->views_[backbuffer_index];
        ...
        ds_attachment.view = render_device->screen_ds_view_[backbuffer_index];
            ...
            for (uint32_t i = 0; i < render_scene->meshes_.size(); i++)
            {
                ...
                for (uint32_t j = 0; j < mesh.primitives_.size(); j++)
                {
                    ...
                    render_context->BindIndexBuffer(
                        render_scene->index_buffer_, render_scene->index_stride_, prim.index_offset_);
        ...
        render_device->Present(backbuffer_index, &present_semaphore, 1);
