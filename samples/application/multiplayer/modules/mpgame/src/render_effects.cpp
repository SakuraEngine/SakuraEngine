...
{
    // initialize queries
    mesh_query = sugoiQ_from_literal(storage, u8"[in]ForwardEffectToken, [in]skr::renderer::MeshComponent");
    draw_mesh_query = sugoiQ_from_literal(storage, u8"[in]ForwardEffectToken, [in]skr::renderer::MeshComponent, [out]skr_render_group_t");
}
...
skr_primitive_draw_packet_t RenderEffectForward::produce_draw_packets(const skr_primitive_draw_context_t* context)
{
    ...
    model_matrices.resize_default(primitiveCount);
...
void initialize_render_effects(SRendererId renderer, skr::render_graph::RenderGraph* renderGraph, skr_vfs_t* resource_vfs)
{
...
void register_render_effects(SRendererId renderer, skr::render_graph::RenderGraph* renderGraph)
{
    skr_renderer_register_render_pass(renderer, forward_pass_name, forward_pass);
}

void finalize_render_effects(SRendererId renderer, skr::render_graph::RenderGraph* renderGraph)
{
