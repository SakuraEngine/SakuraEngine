        ...
        [this, upload_buffer_handle](rg::RenderGraph& g, rg::CopyPassContext& context) {
            ...
            auto uploadVertices = [&](sugoi_chunk_view_t* r_cv) {
                skr::anim::AnimComponent* anims = nullptr;
                {
                    ...
                    anims = sugoi::get_owned_rw<skr::anim::AnimComponent>(r_cv);
                }
...
void RenderPassForward::execute(const skr_primitive_pass_context_t* context, skr::span<const skr_primitive_draw_packet_t> drawcalls)
{
    ...
    skr::FixedVector<char, 64> ButtonText{ "SwitchShadingRate-", sizeof("SwitchShadingRate-") };
    ImGui::Begin("ShadingRate");
    ButtonText.pop_back();
    ButtonText.append(shadingRateNames[shading_rate], 3);
    ButtonText.add(0);
