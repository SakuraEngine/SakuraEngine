...
target("static0")
    set_kind("static")
    add_rules("sakura.dyn_module", {api = "STATIC0", version = engine_version})
    public_dependency("SkrRT", engine_version)
    add_files("test/static0.cpp")

target("dynamic0")
    set_kind("shared")
    add_rules("sakura.dyn_module", {api = "DYNAMIC0", version = engine_version})
    public_dependency("SkrRT", engine_version)
    add_files("test/dynamic0.cpp")

target("dynamic1")
    set_kind("shared")
    add_rules("sakura.dyn_module", {api = "DYNAMIC1", version = engine_version})
    public_dependency("SkrRT", engine_version)
    add_files("test/dynamic1.cpp")

target("dynamic2")
    set_kind("shared")
    add_rules("sakura.dyn_module", {api = "DYNAMIC2", version = engine_version})
    public_dependency("SkrRT", engine_version)
    add_files("test/dynamic2.cpp")

target("dynamic3")
    set_kind("shared")
    add_rules("sakura.dyn_module", {api = "DYNAMIC3", version = engine_version})
    public_dependency("SkrRT", engine_version)
    add_files("test/dynamic3.cpp")

target("ModuleTest")
    set_kind("binary")
    add_rules("sakura.dyn_module", {api = "MODULE_TEST", version = engine_version})
    public_dependency("SkrRT", engine_version)
