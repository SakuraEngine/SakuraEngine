...
TEST_CASE("test fixed sparse array")
{
    ...
    SUBCASE("ctor")
    {
        ...
        REQUIRE_EQ(d.sparse_size(), 6);
        REQUIRE_EQ(d.hole_size(), 0);
        REQUIRE_GE(d.capacity(), kFixedCapacity);
        REQUIRE(d.has_data(0));
        REQUIRE_EQ(d[0], 1);
        REQUIRE(d.has_data(1));
        REQUIRE_EQ(d[1], 1);
        ...
        REQUIRE_EQ(e.sparse_size(), 6);
        REQUIRE_EQ(e.hole_size(), 0);
        REQUIRE_GE(e.capacity(), kFixedCapacity);
        REQUIRE(e.has_data(0));
        REQUIRE_EQ(e[0], 1);
        REQUIRE(e.has_data(1));
        REQUIRE_EQ(e[1], 1);
    ...
    SUBCASE("add")
    {
        ...
        for (int32_t i = 0; i < kFixedCapacity - 7; ++i)
        {
            a.add(i);
        }
        for (int32_t i = 0; i < kFixedCapacity - 7; ++i)
    ...
    SUBCASE("erase")
    {
        ...
        TestSparseArray a(kFixedCapacity), b(kFixedCapacity);
    ...
    SUBCASE("sort")
    {
        static constexpr uint64_t kSparseSizeAfterMove = (kFixedCapacity + 1) / 2;

        TestSparseArray a(kFixedCapacity);
        for (size_t i = 0; i < kFixedCapacity; ++i)
        {
            a[i] = static_cast<u32>(kFixedCapacity - 1 - i);
        }
        a.remove_all_if([](const u32& n) { return n % 2 == 1; });
        REQUIRE_EQ(a.size(), kSparseSizeAfterMove);
        REQUIRE_EQ(a.sparse_size(), kFixedCapacity);
        REQUIRE_EQ(a.hole_size(), kFixedCapacity - kSparseSizeAfterMove);
        REQUIRE_EQ(a.capacity(), kFixedCapacity);
        a.sort();
        REQUIRE_EQ(a.size(), kSparseSizeAfterMove);
        REQUIRE_EQ(a.sparse_size(), kSparseSizeAfterMove);
        REQUIRE_EQ(a.hole_size(), 0);
        REQUIRE_EQ(a.capacity(), kFixedCapacity);
        for (size_t i = 0; i < (kFixedCapacity + 1) / 2; ++i)
        {
            REQUIRE_EQ(a[i], i * 2);
        }
        ...
        REQUIRE_EQ(a.hole_size(), 0);
        REQUIRE_EQ(a.capacity(), kFixedCapacity);
        for (size_t i = 0; i < kSparseSizeAfterMove; ++i)
        {
            REQUIRE_EQ(a[i], (kSparseSizeAfterMove - 1 - i) * 2);
        }
    }

    // [test in sparse array] foreach
    // [test in sparse array] test iterator
}
...
TEST_CASE("test inline sparse array")
{
    ...
    auto                      capacity_of     = [](uint64_t capacity) -> uint64_t { return capacity < kInlineCapacity ? kInlineCapacity : capacity; };
    ...
    SUBCASE("ctor")
    {
        ...
        REQUIRE_EQ(a.sparse_size(), 0);
        REQUIRE_EQ(a.hole_size(), 0);
        REQUIRE_EQ(a.capacity(), capacity_of(capacity_of(0)));
    ...
    SUBCASE("sort")
    {
        ...
        REQUIRE_EQ(a.sparse_size(), 1000);
        REQUIRE_EQ(a.hole_size(), 500);
        REQUIRE_EQ(a.capacity(), capacity_of(1000));
        a.sort();
        REQUIRE_EQ(a.size(), 500);
        REQUIRE_EQ(a.sparse_size(), 500);
        REQUIRE_EQ(a.hole_size(), 0);
