...
TEST_CASE_METHOD(BinarySerdeTests, "primitives")
{
    ...
    skr::binary::Write(&warchive, value);
    skr::binary::Write(&warchive, value2);

    reader.data = skr::span<const uint8_t>(buffer.data(), buffer.size());

    uint64_t readValue  = 0;
    uint64_t readValue2 = 0;
    skr::binary::Read(&rarchive, readValue);
    EXPECT_EQ(value, readValue);
    skr::binary::Read(&rarchive, readValue2);
...
TEST_CASE_METHOD(BinarySerdeTests, "vector")
{
    ...
    arr.add(value);
    arr.add(value2);
    EXPECT_TRUE(skr::binary::Write(&warchive, arr));

    reader.data = skr::span<const uint8_t>(buffer.data(), buffer.size());

    skr::Vector<uint64_t> readArr;
    EXPECT_TRUE(skr::binary::Read(&rarchive, readArr));
...
TEST_CASE_METHOD(BinarySerdeTests, "arr")
{
    ...
    arr.add(value);
    arr.add(value2);
    skr::binary::Write(&warchive, arr);

    reader.data = skr::span<const uint8_t>(buffer.data(), buffer.size());

    skr::Vector<uint64_t> readArr;
    skr::binary::Read(&rarchive, readArr);
...
TEST_CASE_METHOD(BinarySerdeTests, "str")
{
    ...
    skr::binary::Write(&warchive, str);

    reader.data = skr::span<const uint8_t>(buffer.data(), buffer.size());

    skr::String readStr;
    skr::binary::Read(&rarchive, readStr);
