...
void shared_atomic_mutex::lock() SKR_NOEXCEPT
{
    ...
    atomic_fetch_add(&_bitfield, one_unique_thread);
    acquire_unique();
}
...
void shared_atomic_mutex::unlock() SKR_NOEXCEPT
{
    ...
    atomic_fetch_sub(&_bitfield, one_unique_flag + one_unique_thread);
}
...
void shared_atomic_mutex::upgrade() SKR_NOEXCEPT
{
    ...
    atomic_fetch_add(&_bitfield, one_unique_thread - one_shared_thread);
    acquire_unique();
}
...
void shared_atomic_mutex::lock_shared() SKR_NOEXCEPT
{
    ...
    bitfield_t oldval = atomic_load(&_bitfield);
    ...
    do
    {
        ...
        expected = oldval, 
        !atomic_compare_exchange_weak_explicit(&_bitfield, &expected, newval, memory_order_relaxed, memory_order_relaxed)
    ));
}
