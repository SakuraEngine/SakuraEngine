...
{
    return static_cast<Status>(skr_atomic_load_acquire(&status_));
}

ServiceThread::Action ServiceThread::get_action() const SKR_NOEXCEPT
{
    return static_cast<Action>(skr_atomic_load_acquire(&action_));
}
...
uint64_t ServiceThread::request(Action action) SKR_NOEXCEPT
{
    ...
    auto eid = skr_atomic_fetch_add_relaxed(&event_, 1) + 1;
    setAction(action);
    return eid;
}
...
{
    skr_atomic_store_release(&action_, target);
...
void ServiceThread::setStatus(Status target) SKR_NOEXCEPT
{
    ...
    skr_atomic_store_release(&status_, target);
...
{
    const auto ms = skr_atomic_load_relaxed(&sleep_time);
