...
void NamedThread::threadFunc(void* args)
{
    ...
    atomic_store_release(&pSelf->started, true);
    atomic_store_release(&pSelf->alive, true);

    pSelf->func->run();
    
    atomic_store_release(&pSelf->alive, false);
}

AsyncResult NamedThread::start(NamedThreadFunction* pFunc) SKR_NOEXCEPT
{
    if (atomic_load_acquire(&started)) 
    {
    ...
    const auto P = (SThreadPriority)atomic_load_acquire(&priority);
    skr_thread_set_priority(tHandle, P);

    wait_timeout([this]() { return atomic_load_acquire(&started); }, 4);
...
{
    return atomic_load_acquire(&alive);
}

bool NamedThread::has_started() const SKR_NOEXCEPT
{
    return atomic_load_acquire(&started);
}
...
{
    atomic_store_relaxed(&priority, pri);
...
AsyncResult NamedThread::initialize(const NamedThreadDesc& pdesc) SKR_NOEXCEPT
{
    ...
    atomic_store_relaxed(&priority, priority);
    return ASYNC_RESULT_OK;
}
...
AsyncResult NamedThread::finalize() SKR_NOEXCEPT
{
    if (atomic_load_acquire(&started)) 
    {
        ...
        atomic_store_release(&started, false);
    }
    return ASYNC_RESULT_OK;
}
