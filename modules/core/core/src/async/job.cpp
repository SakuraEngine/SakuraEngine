...
public:
    ...
    {
        cond->lock();
        const auto endJobEnqueud = atomic_load_acquire(&is_end_job_queued);
        ...
        atomic_store_release(&jobItem->status, isEndJob ? kJobItemStatusFinishJob : kJobItemStatusWaiting);
        list_runnable.emplace_back(jobItem);

        // end job is queued, only end job is accepted
        atomic_store_release(&is_end_job_queued, isEndJob);

        const auto c = atomic_load_acquire(&waiting_workers_count);
        if (c) 
        {
            cond->signal();
        }
    ...
    void erase(JobItem* jobItem)
    {
        ...
        for (auto i = list_consumed.begin(); i != list_consumed.end(); ++i)
        {
            if ((*i) == jobItem) 
            {
                ...
                atomic_store_release(&jobItem->status, kJobItemStatusNone);
                isFound = true;
                break;
            }
        }
    ...
    JobItem* getRunnableJobItem()
    {
        ...
        {
            atomic_fetch_add_relaxed(&waiting_workers_count, 1);
...
JobResult JobThreadFunctionImpl::run() SKR_NOEXCEPT
{
    ...
    while (false == isEndJobArrived)
    {
        ...
        if (m_item)
        {
            const auto itemStatus = atomic_load_acquire(&m_item->status);
...
{
    return (atomic_load_acquire(&status) == kJobItemStatusNone);
}
...
{
    skr_rw_mutex_acquire_r(&pending_queue_mutex);
    SKR_DEFER({skr_rw_mutex_release_r(&pending_queue_mutex);});
...
JobResult JobQueue::check() SKR_NOEXCEPT
{
    ...
    need_cancel = atomic_load_acquire(&cancel_requested);
    if (need_cancel)
    {
        ...
        atomic_store_release(&cancel_requested, false);
    }
    ...
    while (it != pending_queue.end()) {
        ...
        if ((*it)->is_none()) {
            ...
            skr_rw_mutex_release_w(&pending_queue_mutex);
...
int JobQueue::enqueueCore(JobItem* jobItem, bool isEndJob) SKR_NOEXCEPT
{
    ...
    if (atomic_load_acquire(&jobItem->status) != kJobItemStatusNone)
    {
...
JobResult JobQueue::cancel_all_items() SKR_NOEXCEPT
{
    skr_rw_mutex_acquire_w(&pending_queue_mutex);
    SKR_DEFER({skr_rw_mutex_release_w(&pending_queue_mutex);});
    atomic_store_release(&cancel_requested, true);
    return ASYNC_RESULT_OK;
}
