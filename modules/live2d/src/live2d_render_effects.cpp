        ...
        auto counterF = [&](dual_chunk_view_t* r_cv) {
            ...
            for (uint32_t i = 0; i < r_cv->count; i++)
            {
                if (models[i].vram_future.is_ready())
                {
                    ...
                    push_constants.try_add(render_model).value().resize(0);
            ...
            for (uint32_t i = 0; i < r_cv->count; i++)
            {
                if (models[i].vram_future.is_ready())
                {
                    ...
                    if (!mask_push_constants.contains(render_model))
                        mask_push_constants.try_add(render_model);
                        ...
                        for (uint32_t j = 0; j < mask_list->GetSize(); j++)
                        {
                            ...
                            if (clipping_context && !use_high_precision_mask && clipping_context->_isUsing)
                            {
                                ...
                                for (csmInt32 ctx = 0; ctx < clipDrawCount; ctx++)
                                {
                                    ...
                                    sorted_mask_drawable_lists.try_add(render_model).value().emplace(clipDrawIndex);
...
protected:
    ...
    void updateModelMotion(skr::render_graph::RenderGraph* render_graph, skr_live2d_render_model_id render_model)
    {
        ...
        if (!motion_timers.contains(render_model))
            motion_timers.try_add(render_model);
