...
{
    return (EState)atomic_load_relaxed((uint32_t*)&_state);
}
...
{
    atomic_store_release((uint32_t*)&_state, (uint32_t)target);
}
...
{
    return atomic_load_relaxed(&_async_is_okey);
}
...
void SkrImageUploadTask::from_image(const SkrImageData& image)
{
    ...
    request->add_callback(SKR_IO_STAGE_COMPLETED, +[](skr_io_future_t* future, skr_io_request_t* request, void* usrdata) {
        ...
        atomic_store_release(&task->_async_is_okey, 1);
    }, this);
