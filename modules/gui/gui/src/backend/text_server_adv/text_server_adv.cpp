...
const bool kHasRenderer_HACK()
{
    return true;
}
const bool kHasSystemFont()
{
    return false;
}

const String kGetLocale()
{
...
_WHY_GODOT_INLINE_ void TextServerAdvanced::_insert_feature(const StringName& p_name, int32_t p_tag, Variant::Type p_vtype, bool p_hidden)
{
    FeatureInfo fi;
    fi.name   = p_name;
    fi.vtype  = p_vtype;
...
/*************************************************************************/
_WHY_GODOT_INLINE_ TextServerAdvanced::FontTexturePosition TextServerAdvanced::find_texture_pos_for_glyph(FontForSizeAdvanced* p_data, int p_color_size, ImageFormat p_image_format, int p_width, int p_height, bool p_msdf) const
{
    ...
    {
        if (p_image_format != ct[i].atlas->format())
        {
            continue;
        }
        if (mw > ct[i].texture_w || mh > ct[i].texture_h)
        { // Too big for this texture.
            continue;
        }
    ...
    if (ret.index == -1)
    {
        ...
        {
            // Zero texture.
            uint8_t* w = tex.atlas->data_for_write().ptrw();
            ERR_FAIL_COND_V(texsize * texsize * p_color_size > tex.atlas->data().size(), ret);
            // Initialize the texture to all-white pixels to prevent artifacts when the
            // font is displayed at a non-default scale with filtering enabled.
            if (p_color_size == 1)
            {
                for (int i = 0; i < texsize * texsize * p_color_size; i += 1)
                { // FORMAT_LA8, BW font.
                    // w[i + 0] = 255;
                    w[i + 0] = 0;
                }
            ...
            else if (p_color_size == 4)
            {
                ...
                { // FORMAT_RGBA8, Color font, Multichannel(+True) SDF.
                    w[i + 0] = 255;
                    w[i + 1] = 255;
                    w[i + 2] = 255;
                    w[i + 3] = 0;
        ...
        ret         = p_data->textures[idx].pack_rect(idx, mh, mw);
    }
...
#ifdef MODULE_FREETYPE_ENABLED
_WHY_GODOT_INLINE_ TextServerAdvanced::FontGlyph TextServerAdvanced::rasterize_bitmap(FontForSizeAdvanced* p_data, int p_rect_margin, FT_Bitmap bitmap, int yofs, int xofs, const Vector2& advance, bool p_bgra) const
{
    int w          = bitmap.width;
    int h          = bitmap.rows;
    int color_size = 1;
    ...
    ShelfPackTexture& tex = p_data->textures[tex_pos.index];

    {
        uint8_t* wr = tex.atlas->data_for_write().ptrw();
        ...
        for (int i = 0; i < h; i++)
        {
            for (int j = 0; j < w; j++)
            {
                ...
                ERR_FAIL_COND_V(ofs >= tex.atlas->data().size(), FontGlyph());
                switch (bitmap.pixel_mode)
                {
                    case FT_PIXEL_MODE_MONO: {
...
/* Font Cache                                                            */
/*************************************************************************/
_WHY_GODOT_INLINE_ bool TextServerAdvanced::_ensure_glyph(FontAdvanced* p_font_data, const Vector2i& p_size, int32_t p_glyph) const
{
    ...
    if (glyph_index == 0)
    { // Non graphical or invalid glyph, do not render.
...
_WHY_GODOT_INLINE_ bool TextServerAdvanced::_ensure_cache_for_size(FontAdvanced* p_font_data, const Vector2i& p_size) const
{
...
_WHY_GODOT_INLINE_ void TextServerAdvanced::_font_clear_cache(FontAdvanced* p_font_data)
{
    MutexLock ftlock(ft_mutex);

    for (const auto& E : p_font_data->cache)
    {
        memdelete(E.second);
    }
...
void TextServerAdvanced::_font_set_generate_mipmaps(const RID& p_font_rid, bool p_generate_mipmaps)
{
    ...
    if (fd->mipmaps != p_generate_mipmaps)
    {
        for (auto& E : fd->cache)
        {
            for (int i = 0; i < E.second->textures.size(); i++)
            {
                E.second->textures[i].atlas->mark_dirty();
                // TODO. gen mipmaps
            }
        }
...
void TextServerAdvanced::_font_set_variation_coordinates(const RID& p_font_rid, const VariationCoordinates& p_variation_coordinates)
{
    FontAdvanced* fd = font_owner.get_or_null(p_font_rid);
    ERR_FAIL_COND(!fd);
...
VariationCoordinates TextServerAdvanced::_font_get_variation_coordinates(const RID& p_font_rid) const
{
    FontAdvanced* fd = font_owner.get_or_null(p_font_rid);
    ERR_FAIL_COND_V(!fd, VariationCoordinates());
...
void TextServerAdvanced::_font_set_texture_image(const RID& p_font_rid, const Vector2i& p_size, int64_t p_texture_index, const Ref<FontAtlasImage>& p_image)
{
    FontAdvanced* fd = font_owner.get_or_null(p_font_rid);
    ERR_FAIL_COND(!fd);
    ...
    ShelfPackTexture& tex = fd->cache[size]->textures[p_texture_index];
    tex.atlas             = p_image;
}

Ref<FontAtlasImage> TextServerAdvanced::_font_get_texture_image(const RID& p_font_rid, const Vector2i& p_size, int64_t p_texture_index) const
{
    FontAdvanced* fd = font_owner.get_or_null(p_font_rid);
    ERR_FAIL_COND_V(!fd, Ref<FontAtlasImage>());

    MutexLock lock(fd->mutex);
    Vector2i  size = _get_size_outline(fd, p_size);
    ERR_FAIL_COND_V(!_ensure_cache_for_size(fd, size), Ref<FontAtlasImage>());
    ERR_FAIL_INDEX_V(p_texture_index, fd->cache[size]->textures.size(), Ref<FontAtlasImage>());
...
Vector2 TextServerAdvanced::_font_get_glyph_advance(const RID& p_font_rid, int64_t p_size, int64_t p_glyph) const
{
    ...
    const auto advance = gl.find(p_glyph | mod)->second.advance;
    if (fd->msdf)
    {
        return (advance + ea) * (double)p_size / (double)fd->msdf_source_size;
    }
...
Vector2 TextServerAdvanced::_font_get_glyph_offset(const RID& p_font_rid, const Vector2i& p_size, int64_t p_glyph) const
{
    ...
    const HashMap<int32_t, FontGlyph>& gl   = fd->cache[size]->glyph_map;
    const auto                         rect = gl.find(p_glyph | mod)->second.rect;
    if (fd->msdf)
    {
        return rect.position * (double)p_size.x / (double)fd->msdf_source_size;
    }
    else
    {
...
Size2 TextServerAdvanced::_font_get_glyph_texture_size(const RID& p_font_rid, const Vector2i& p_size, int64_t p_glyph) const
{
    ...
    const HashMap<int32_t, FontGlyph>& gl          = fd->cache[size]->glyph_map;
    auto                               texture_idx = gl.find(p_glyph | mod)->second.texture_idx;
    ERR_FAIL_COND_V(texture_idx < -1 || texture_idx >= fd->cache[size]->textures.size(), Size2());
    ...
    if (kHasRenderer_HACK())
    {
        if (texture_idx != -1)
        {
            ...
            auto size = tex.atlas->size();
            return { (real_t)size.width, (real_t)size.height };
        }
    }
...
bool TextServerAdvanced::_font_get_glyph_contours(const RID& p_font_rid, int64_t p_size, int64_t p_index, Vector<Vector3>& r_points, Vector<int32_t>& r_contours, bool& r_orientation) const
{
    FontAdvanced* fd = font_owner.get_or_null(p_font_rid);
...
Vector2 TextServerAdvanced::_font_get_kerning(const RID& p_font_rid, int64_t p_size, const Vector2i& p_glyph_pair) const
{
    ...
    const auto                        glyph_kern = kern.find(p_glyph_pair)->second;

    if (kern.has(p_glyph_pair))
    {
        if (fd->msdf)
        {
            return glyph_kern * (double)p_size / (double)fd->msdf_source_size;
        }
        else
        {
            return glyph_kern;
...
bool TextServerAdvanced::_font_has_char(const RID& p_font_rid, int64_t p_char) const
{
    ...
    FontForSizeAdvanced* at_size = fd->cache.begin()->second;
...
String TextServerAdvanced::_font_get_supported_chars(const RID& p_font_rid) const
{
    ...
    if (at_size)
    {
        ...
        for (const auto& E : gl)
        {
            chars = chars + String::chr(E.first);
        }
    }
    return chars;
}
...
void TextServerAdvanced::_font_draw_glyph(const RID& p_font_rid, const RID& p_canvas, int64_t p_size, const Vector2& p_pos, int64_t p_index, const Color& p_color) const
{
    ...
    if (!fd->msdf && fd->cache[size]->face)
    {
        ...
        if ((fd->subpixel_positioning == SUBPIXEL_POSITIONING_ONE_QUARTER) || (fd->subpixel_positioning == SUBPIXEL_POSITIONING_AUTO && size.x <= SUBPIXEL_POSITIONING_ONE_QUARTER_MAX_SIZE))
        {
            int xshift = (int)(::floor(4 * (p_pos.x + 0.125)) - 4 * ::floor(p_pos.x + 0.125));
            index      = index | (xshift << 27);
        }
        else if ((fd->subpixel_positioning == SUBPIXEL_POSITIONING_ONE_HALF) || (fd->subpixel_positioning == SUBPIXEL_POSITIONING_AUTO && size.x <= SUBPIXEL_POSITIONING_ONE_HALF_MAX_SIZE))
        {
            int xshift = (int)(::floor(2 * (p_pos.x + 0.25)) - 2 * ::floor(p_pos.x + 0.25));
            index      = index | (xshift << 27);
        }
    }
#endif
...
#ifdef MODULE_FREETYPE_ENABLED
            if (fd->cache[size]->face && (tex.atlas->format() == ImageFormat::RGBA8) && !lcd_aa && !fd->msdf)
            {
            ...
            if (kHasRenderer_HACK())
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        skr::gui::Swizzle swizzle;
                        switch (image_texture->format())
                        {
                            case FontAtlasImage::Format::RGBA8:
                                swizzle = { skr::gui::ESwizzleChannel::R, skr::gui::ESwizzleChannel::G, skr::gui::ESwizzleChannel::B, skr::gui::ESwizzleChannel::A };
                                break;
                            case FontAtlasImage::Format::RGB8:
                                swizzle = { skr::gui::ESwizzleChannel::R, skr::gui::ESwizzleChannel::G, skr::gui::ESwizzleChannel::B, skr::gui::ESwizzleChannel::One };
                                break;
                            case FontAtlasImage::Format::L8:
                                swizzle = { skr::gui::ESwizzleChannel::R, skr::gui::ESwizzleChannel::R, skr::gui::ESwizzleChannel::R, skr::gui::ESwizzleChannel::R };
                                break;
                        ...
                        // resolve UV
                        Size2      tex_size        = { (real_t)image_texture->size().width, (real_t)image_texture->size().height };
...
PackedStringArray TextServerAdvanced::_font_get_language_support_overrides(const RID& p_font_rid)
{
    ...
    for (const auto& E : fd->language_support_overrides)
    {
...
PackedStringArray TextServerAdvanced::_font_get_script_support_overrides(const RID& p_font_rid)
{
    ...
    MutexLock         lock(fd->mutex);
    PackedStringArray out;
    for (const auto& E : fd->script_support_overrides)
    {
        out.push_back(E.first);
    }
    return out;
...
void TextServerAdvanced::_font_set_opentype_feature_overrides(const RID& p_font_rid, const TextServerFeatures& p_overrides)
{
    FontAdvanced* fd = font_owner.get_or_null(p_font_rid);
    ERR_FAIL_COND(!fd);
...
TextServerFeatures TextServerAdvanced::_font_get_opentype_feature_overrides(const RID& p_font_rid) const
{
    FontAdvanced* fd = font_owner.get_or_null(p_font_rid);
    ERR_FAIL_COND_V(!fd, TextServerFeatures());

    MutexLock lock(fd->mutex);
    return fd->feature_overrides;
}

TextServerFeatures TextServerAdvanced::_font_supported_feature_list(const RID& p_font_rid) const
{
    FontAdvanced* fd = font_owner.get_or_null(p_font_rid);
    ERR_FAIL_COND_V(!fd, TextServerFeatures());
    ...
    MutexLock lock(fd->mutex);
    Vector2i  size = _get_size(fd, 16);
    ERR_FAIL_COND_V(!_ensure_cache_for_size(fd, size), TextServerFeatures());
    return fd->supported_features;
}

TextServerVariants TextServerAdvanced::_font_supported_variation_list(const RID& p_font_rid) const
{
    FontAdvanced* fd = font_owner.get_or_null(p_font_rid);
    ERR_FAIL_COND_V(!fd, TextServerVariants());

    MutexLock lock(fd->mutex);
    Vector2i  size = _get_size(fd, 16);
    ERR_FAIL_COND_V(!_ensure_cache_for_size(fd, size), TextServerVariants());
...
void TextServerAdvanced::_shaped_set_span_update_font(const RID& p_shaped, int64_t p_index, const TypedArray<RID>& p_fonts, int64_t p_size, const TextServerFeatures& p_opentype_features)
{
    ShapedTextDataAdvanced* sd = shaped_owner.get_or_null(p_shaped);
    ERR_FAIL_COND(!sd);
                                                    ...
                                                    const Size2& p_size, InlineAlignment p_inline_align, double p_baseline)
{
    ...
    if (sd->valid)
    {
        ...
        for (int i = 0; i < sd_size; i++)
        {
            ...
            if (gl.count == 1)
            {
                for (const auto& E : sd->objects)
                {
                    if (E.second.pos == gl.start)
                    {
                        key = E.first;
                        break;
                    }
            ...
            else
            {
                if (gl.font_rid.is_valid())
                {
                    ...
                    {
                        sd->ascent  = MAX(sd->ascent, ::round(_font_get_glyph_advance(gl.font_rid, gl.font_size, gl.index).x * 0.5));
                        sd->descent = MAX(sd->descent, ::round(_font_get_glyph_advance(gl.font_rid, gl.font_size, gl.index).x * 0.5));
                    }
                ...
                else if (sd->preserve_invalid || (sd->preserve_control && is_control(gl.index)))
                {
                    ...
                    {
                        sd->ascent  = MAX(sd->ascent, ::round(get_hex_code_box_size(gl.font_size, gl.index).x * 0.5));
                        sd->descent = MAX(sd->descent, ::round(get_hex_code_box_size(gl.font_size, gl.index).x * 0.5));
                    }
                }
...
void TextServerAdvanced::_realign(ShapedTextDataAdvanced* p_sd) const
{
    ...
    {
        // SKR_UNIMPLEMENTED_FUNCTION();
        if ((E.second.pos >= p_sd->start) && (E.second.pos < p_sd->end))
        {
            if (p_sd->orientation == ORIENTATION_HORIZONTAL)
            {
                switch (E.second.inline_align & INLINE_ALIGNMENT_TEXT_MASK)
                {
                    case INLINE_ALIGNMENT_TO_TOP: {
                        E.second.rect.position.y = -p_sd->ascent;
                    }
                    break;
                    case INLINE_ALIGNMENT_TO_CENTER: {
                        E.second.rect.position.y = (-p_sd->ascent + p_sd->descent) / 2;
                    }
                    break;
                    case INLINE_ALIGNMENT_TO_BASELINE: {
                        E.second.rect.position.y = 0;
                    }
                    break;
                    case INLINE_ALIGNMENT_TO_BOTTOM: {
                        E.second.rect.position.y = p_sd->descent;
                    }
                    break;
                }
                switch (E.second.inline_align & INLINE_ALIGNMENT_IMAGE_MASK)
                {
                    case INLINE_ALIGNMENT_BOTTOM_TO: {
                        E.second.rect.position.y -= E.second.rect.size.y;
                    }
                    break;
                    case INLINE_ALIGNMENT_CENTER_TO: {
                        E.second.rect.position.y -= E.second.rect.size.y / 2;
                    }
                    break;
                    case INLINE_ALIGNMENT_TO_BASELINE: {
                        E.second.rect.position.y -= E.second.baseline;
                    }
                    break;
                ...
                full_ascent  = MAX(full_ascent, -E.second.rect.position.y);
                full_descent = MAX(full_descent, E.second.rect.position.y + E.second.rect.size.y);
            }
            else
            {
                switch (E.second.inline_align & INLINE_ALIGNMENT_TEXT_MASK)
                {
                    case INLINE_ALIGNMENT_TO_TOP: {
                        E.second.rect.position.x = -p_sd->ascent;
                    }
                    break;
                    case INLINE_ALIGNMENT_TO_CENTER: {
                        E.second.rect.position.x = (-p_sd->ascent + p_sd->descent) / 2;
                    }
                    break;
                    case INLINE_ALIGNMENT_TO_BASELINE: {
                        E.second.rect.position.x = 0;
                    }
                    break;
                    case INLINE_ALIGNMENT_TO_BOTTOM: {
                        E.second.rect.position.x = p_sd->descent;
                    }
                    break;
                }
                switch (E.second.inline_align & INLINE_ALIGNMENT_IMAGE_MASK)
                {
                    case INLINE_ALIGNMENT_BOTTOM_TO: {
                        E.second.rect.position.x -= E.second.rect.size.x;
                    }
                    break;
                    case INLINE_ALIGNMENT_CENTER_TO: {
                        E.second.rect.position.x -= E.second.rect.size.x / 2;
                    }
                    break;
                    case INLINE_ALIGNMENT_TO_BASELINE: {
                        E.second.rect.position.x -= E.second.baseline;
                    }
                    break;
                    case INLINE_ALIGNMENT_TOP_TO: {
                        // NOP
                ...
                full_ascent  = MAX(full_ascent, -E.second.rect.position.x);
                full_descent = MAX(full_descent, E.second.rect.position.x + E.second.rect.size.x);
            }
        }
    }
            ...
            for (int i = 0; i < bidi_run_count; i++)
            {
                ...
                for (int j = 0; j < sd_size; j++)
                {
                    if ((sd_glyphs[j].start >= bidi_run_start) && (sd_glyphs[j].end <= bidi_run_end))
                    {
                        ...
                        if (gl.count == 1)
                        {
                            for (const auto& E : p_sd->objects)
                            {
                                if (E.second.pos == gl.start)
                                {
                                    find_embedded          = true;
                                    key                    = E.first;
                                    p_new_sd->objects[key] = E.second;
                                    break;
                                }
                            }
                        }
                        ...
                        else
                        {
                            if (gl.font_rid.is_valid())
                            {
                                ...
                                {
                                    p_new_sd->ascent  = MAX(p_new_sd->ascent, ::round(_font_get_glyph_advance(gl.font_rid, gl.font_size, gl.index).x * 0.5));
                                    p_new_sd->descent = MAX(p_new_sd->descent, ::round(_font_get_glyph_advance(gl.font_rid, gl.font_size, gl.index).x * 0.5));
                                }
                            }
                            else if (p_new_sd->preserve_invalid || (p_new_sd->preserve_control && is_control(gl.index)))
                            {
                                ...
                                {
                                    p_new_sd->ascent  = MAX(p_new_sd->ascent, ::round(get_hex_code_box_size(gl.font_size, gl.index).x * 0.5));
                                    p_new_sd->descent = MAX(p_new_sd->descent, ::round(get_hex_code_box_size(gl.font_size, gl.index).x * 0.5));
                                }
                            }
...
double TextServerAdvanced::_shaped_text_fit_to_width(const RID& p_shaped, double p_width, BitField<TextServer::JustificationFlag> p_jst_flags)
{
    ...
    if (::floor(p_width) < ::floor(justification_width))
    {
    ...
    return ::ceil(justification_width);
}
...
void TextServerAdvanced::_shaped_text_overrun_trim_to_width(const RID& p_shaped_line, double p_width, BitField<TextServer::TextOverrunFlag> p_trim_flags)
{
    ...
    {
        const auto& fonts = spans[spans.size() - 1].fonts;
...
_WHY_GODOT_INLINE_ int64_t _generate_kashida_justification_opportunies(const String& p_data, int64_t p_start, int64_t p_end)
{
...
Glyph TextServerAdvanced::_shape_single_glyph(ShapedTextDataAdvanced* p_sd, char32_t p_char, hb_script_t p_script, hb_direction_t p_direction, const RID& p_font, int64_t p_font_size)
{
    ...
    if (glyph_count > 0)
    {
        if (p_sd->orientation == ORIENTATION_HORIZONTAL)
        {
            ...
            {
                gl.advance = ::round((double)glyph_pos[0].x_advance / (64.0 / scale) + _get_extra_advance(p_font, p_font_size));
            }
        }
        else
        {
            gl.advance = -::round((double)glyph_pos[0].y_advance / (64.0 / scale));
        }
        gl.count = 1;
        ...
        {
            gl.x_off = ::round((double)glyph_pos[0].x_offset / (64.0 / scale));
        }
        gl.y_off = -::round((double)glyph_pos[0].y_offset / (64.0 / scale));
...
_WHY_GODOT_INLINE_ void TextServerAdvanced::_add_features(const TextServerFeatures& p_source, Vector<hb_feature_t>& r_ftrs)
{
                ...
                if ((sd->script_iter->script_ranges[j].start < bidi_run_end) && (sd->script_iter->script_ranges[j].end > bidi_run_start))
                {
                    ...
                    for (int k = spn_from; k != spn_to; k += spn_delta)
                    {
                        ...
                        else
                        {
                            ...
                            _shape_run(sd, MAX(sd->spans[k].start - sd->start, script_run_start),
                                       MIN(sd->spans[k].end - sd->start, script_run_end), sd->script_iter->script_ranges[j].script,
                                       bidi_run_direction, fonts, k, 0, 0, 0);
                        }
                    }
                }
            }
...
Vector<Variant> TextServerAdvanced::_shaped_text_get_objects(const RID& p_shaped) const
{
    Vector<Variant>               ret;
    const ShapedTextDataAdvanced* sd = shaped_owner.get_or_null(p_shaped);
...
double TextServerAdvanced::_shaped_text_get_width(const RID& p_shaped) const
{
    ...
    return ::ceil(sd->text_trimmed ? sd->width_trimmed : sd->width);
}
...
{
    const StringName lang = (p_language.is_empty()) ? kGetLocale() : p_language;
...
int64_t TextServerAdvanced::_is_confusable(const String& p_string, const PackedStringArray& p_dict) const
{
    ...
    for (int i = 0; i < p_dict.size(); i++)
    {
        ...
        skeletons[i]      = (UChar*)memalloc(++len * sizeof(UChar));
        status            = U_ZERO_ERROR;
        uspoof_getSkeleton(sc_conf, 0, word.get_data(), -1, skeletons[i], len, &status);
    }
...
String TextServerAdvanced::_string_to_upper(const String& p_string, const String& p_language) const
{
    ...
    const String lang = (p_language.is_empty()) ? kGetLocale() : p_language;
