...
/* Barrier implementation because Mac OSX doesn't have thread_barrier.
...
static int thread_barrier_init(thread_barrier_t *barrier, const thread_barrierattr_t *attr, unsigned int count)
{
	(void) attr;
...
static void free_hid_device(hid_device *dev)
{
	...
	sakura_free(dev->input_report_buf);
...
{
	size_t len = wcslen(s);
	wchar_t *ret = (wchar_t*) sakura_malloc((len+1)*sizeof(wchar_t));
	wcscpy(ret, s);
...
static struct hid_device_info *create_device_info_with_usage(IOHIDDeviceRef dev, int32_t usage_page, int32_t usage)
{
	...
	cur_dev = (struct hid_device_info *)sakura_calloc(1, sizeof(struct hid_device_info));
	if (cur_dev == NULL) {
		return NULL;
	}
	...
	if (res == KERN_SUCCESS) {
		...
		cur_dev->path = sakura_calloc(1, 32);
	...
	if (cur_dev->path == NULL) {
		...
		cur_dev->path = (char*)sakura_calloc(1, sizeof *cur_dev->path);
	}
...
struct hid_device_info  HID_API_EXPORT *hid_enumerate(unsigned short vendor_id, unsigned short product_id)
{
	...
	IOHIDDeviceRef *device_array = (IOHIDDeviceRef*) sakura_calloc(num_devices, sizeof(IOHIDDeviceRef));
                         ...
                         uint8_t *report, CFIndex report_length)
{
	...
	rpt = (struct input_report*) sakura_calloc(1, sizeof(struct input_report));
	rpt->data = (uint8_t*) sakura_calloc(1, report_length);
...
hid_device * HID_API_EXPORT hid_open_path(const char *path)
{
	...
	if (ret == kIOReturnSuccess) {
		...
		dev->input_report_buf = (uint8_t*) sakura_calloc(dev->max_input_report_len, sizeof(uint8_t));
...
int HID_API_EXPORT hid_read_timeout(hid_device *dev, unsigned char *data, size_t length, int milliseconds)
{
	...
	if (milliseconds == -1) {
		...
		res = cond_timedwait(dev, &dev->condition, &dev->mutex, milliseconds);
		if (res == 0)
			bytes_read = return_data(dev, data, length);
