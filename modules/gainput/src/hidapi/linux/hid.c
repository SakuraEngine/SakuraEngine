...
/* C */
#include <stdio.h>    /* vsnprintf, snprintf, sscanf */
#include <string.h>   /* strerror, memset, strlen, strcpy, strtok_r, strchr, strcmp */
#include <stdlib.h>   /* mbstowcs, strtol */
#include <locale.h>   /* setlocale, LC_CTYPE */
#include <errno.h>    /* errno, EINVAL, EAGAIN, EINPROGRESS */

/* Unix */
#include <unistd.h>   /* read, write, close */
#include <sys/stat.h> /* fstat */
#include <fcntl.h>    /* open, O_RDONLY, O_RDWR, O_CLOEXEC */
#include <poll.h>     /* struct pollfd, POLLIN, poll, POLLERR, POLLHUP, POLLNVAL */
...
{
	hid_device *dev = (hid_device*) sakura_calloc(1, sizeof(hid_device));
	dev->device_handle = -1;
	dev->blocking = 1;
...
static wchar_t* DuplicateWideStr(wchar_t* str)
{
	...
	wchar_t* dup = (wchar_t*)sakura_malloc(len * sizeof *dup);
	wcscpy(dup, str);
	return dup;
}
...
static wchar_t *utf8_to_wchar_t(const char *utf8)
{
	...
	if (utf8) {
		...
		if ((size_t) -1 == wlen) {
			return DuplicateWideStr(L"");
		}
		ret = (wchar_t*) sakura_calloc(wlen+1, sizeof(wchar_t));
...
static void register_global_error(const char *msg)
{
	if (last_global_error_str)
		sakura_free(last_global_error_str);
...
static void register_device_error(hid_device *dev, const char *msg)
{
	if (dev->last_error_str)
		sakura_free(dev->last_error_str);
...
static int get_hid_report_descriptor_from_sysfs(const char *sysfs_path, struct hidraw_report_descriptor *rpt_desc)
{
	...
	char* rpt_path = (char*) sakura_calloc(1, rpt_path_len);
	snprintf(rpt_path, rpt_path_len, "%s/device/report_descriptor", sysfs_path);

	res = get_hid_report_descriptor(rpt_path, rpt_desc);
	sakura_free(rpt_path);

	return res;
}

static char* DuplicateStr(char const* str)
{
	uint32_t len = strlen(str);
	char* dup = (char*)sakura_malloc(len * sizeof *dup);
	strcpy(dup, str);
	return dup;
}
	...
	while (line != NULL) {
		...
		*value = '\0';
		...
		if (strcmp(key, "HID_ID") == 0) {
			...
			*product_name_utf8 = DuplicateStr(value);
			found_name = 1;
		} else if (strcmp(key, "HID_UNIQ") == 0) {
			/* The caller has to free the serial number */
			*serial_number_utf8 = DuplicateStr(value);
			found_serial = 1;
		}
...
static int get_device_string(hid_device *dev, enum device_string_id key, wchar_t *string, size_t maxlen)
{
	...
	sakura_free(serial_number_utf8);
	sakura_free(product_name_utf8);
...
struct hid_device_info  HID_API_EXPORT *hid_enumerate(unsigned short vendor_id, unsigned short product_id)
{
	...
	udev_list_entry_foreach(dev_list_entry, devices) {
		...
		if ((vendor_id == 0x0 || vendor_id == dev_vid) &&
			...
			tmp = (struct hid_device_info*) sakura_calloc(1, sizeof(struct hid_device_info));
			if (cur_dev) {
				cur_dev->next = tmp;
			}
			else {
			...
			/* Fill out the record */
			cur_dev->next = NULL;
			cur_dev->path = dev_path? DuplicateStr(dev_path): NULL;
			...
			switch (bus_type) {
				case BUS_USB:
					...
					if (!usb_dev) {
						...
						cur_dev->manufacturer_string = DuplicateWideStr(L"");
			...
			if (result >= 0) {
				...
				while (!get_next_hid_usage(report_desc.value, report_desc.size, &pos, &page, &usage)) {
					...
					/* Update fields */
					cur_dev->path = DuplicateStr(dev_path);
					cur_dev->vendor_id = dev_vid;
					cur_dev->product_id = dev_pid;
					cur_dev->serial_number = prev_dev->serial_number? DuplicateWideStr(prev_dev->serial_number): NULL;
					cur_dev->release_number = prev_dev->release_number;
					cur_dev->interface_number = prev_dev->interface_number;
					cur_dev->manufacturer_string = prev_dev->manufacturer_string? DuplicateWideStr(prev_dev->manufacturer_string): NULL;
					cur_dev->product_string = prev_dev->product_string? DuplicateWideStr(prev_dev->product_string): NULL;
					cur_dev->usage_page = page;
					cur_dev->usage = usage;
				}
			}
...
void  HID_API_EXPORT hid_free_enumeration(struct hid_device_info *devs)
{
	...
	while (d) {
		...
		sakura_free(d->path);
		sakura_free(d->serial_number);
		sakura_free(d->manufacturer_string);
		sakura_free(d->product_string);
		sakura_free(d);
		d = next;
	}
}
