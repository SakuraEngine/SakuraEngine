...
struct PS5Controller
{
    ...
    uint8_t useDefaultLEDColors : 1; // 1b
    uint8_t __pad : 2;
    uint8_t ledColor[3]; // rgb
    uint8_t rumbleStrength[2]; // lr
...
static int HIDUpdatePS5Controller(HIDController* controller, gainput::InputDeltaState * state)
{
    ...
    {
        SKR_ASSERT(size <= PS5_PACKET_SIZE_BLUETOOTH_IN);
        ...
        if (*data == PS5_REP_USB_IN)
        {
            ...
            HIDParsePS5CInputReport(con, (PS5CInputReport*)(data + 1), state);
    }
