...
typedef struct LinearAllocator
{
	...
	inline void Free(size_t size)
	{
		if (offset < size)
		{
			SKR_LOG_ERROR("offset(%d) < size(%d)", offset, size);
			return;
		}
		offset -= size;
	}
...
static void PrintControllerInfo(const DIDEVICEINSTANCE* pdidInstance)
{
	...
	SKR_LOG_DEBUG("GamePad Name: %s , GUID: %s, Product Id: %d, Vendor Id: %d", pdidInstance->tszProductName, guid_string, product, vendor);
}
...
class DirectInputInitializer
{
	...
	bool Init(void* _hwnd)
	{
		...
		if (FAILED(re))
		{
			SKR_LOG_ERROR("CoCreateInstance failed for gamePad %08x", re);
			return false;
		}
		if (!gamePads.dinput)
		{
			SKR_LOG_ERROR("gamePads.dinput is null");
			return false;
		}
		//initialize
...
class GainputInputDirectInputPadWin
{
			...
			if (result == size)
			{
				...
				if (raw.header.dwType == RIM_TYPEHID)
				{
					if (!gamepadInfo.assignedHid)
					{
						...
						if (result != 0)
						{
							SKR_LOG_ERROR("result(%d)!=0", result);
							allocator.Free(size);
							return;
						}
						...
						if (result == -1)
						{
							SKR_LOG_ERROR("result(%d)==-1", result);
						}
					}
				}
			}
	...
	bool SetRumbleEffectFeedback(const ControllerFeedback& data)
	{
		...
		{
			SKR_LOG_ERROR("gamepadInfo.hidDevice(%d) == INVALID_HANDLE_VALUE", gamepadInfo.hidDevice);
			return false;
		}
		...
		if (result != TRUE)
		{
			SKR_LOG_ERROR("result(%d) != TRUE", result);
			return false;
		}
		if (bytes_written != ARRAYSIZE(buf))
		{
			SKR_LOG_ERROR("bytes_written(%d) != ARRAYSIZE(buf)(%d)", bytes_written, ARRAYSIZE(buf));
			return false;
		}
		return result;
	}
	...
	void DInputBuffered(InputDeltaState* delta, InputState& state_, InputDevice& device_)
	{
		if (!gamepadInfo.interfacePtr)
		{
			SKR_ASSERT(gamepadInfo.interfacePtr && "GamePad Interface Ptr is null");
			return;
		}
