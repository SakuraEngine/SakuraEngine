...
{
    if constexpr (is_complete_serde_v<skr::binary::WriteTrait<T>>)
        return [](const void* address, skr_binary_writer_t* archive) {
            T* ptr = (T*)address;
            return skr::binary::WriteTrait<T>::Write(archive, *ptr);
        };
    return nullptr;
}
...
{
    if constexpr (is_complete_serde_v<skr::json::WriteTrait<T>>)
        return [](const void* address, skr_json_writer_t* archive) {
            T* ptr = (T*)address;
            return skr::json::WriteTrait<T>::Write(archive, *ptr);
        };
    return nullptr;
}
