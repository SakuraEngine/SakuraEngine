...
RUNTIME_API void log_log(int level, const char* file, int line, const char* fmt, ...);
    ...
    #define SKR_LOG_FMT_INFO(...) log_log(SKR_LOG_LEVEL_INFO, __FILE__, __LINE__, "%s", fmt::format(__VA_ARGS__).c_str())
    #define SKR_LOG_FMT_WARN(...) log_log(SKR_LOG_LEVEL_WARN, __FILE__, __LINE__, "%s", fmt::format(__VA_ARGS__).c_str())
    #define SKR_LOG_FMT_ERROR(...) log_log(SKR_LOG_LEVEL_ERROR, __FILE__, __LINE__, "%s", fmt::format(__VA_ARGS__).c_str())
    #define SKR_LOG_FMT_FATAL(...) log_log(SKR_LOG_LEVEL_FATAL, __FILE__, __LINE__, "%s", fmt::format(__VA_ARGS__).c_str())
#endif