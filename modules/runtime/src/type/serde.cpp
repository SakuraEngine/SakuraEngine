    ...
    else
    {
        switch (type)
        {
            ...
            case SKR_TYPE_CATEGORY_I32:
                std::from_chars(str.data(), str.data() + str.size(), *(int32_t*)dst);
                break;
            case SKR_TYPE_CATEGORY_I64:
                std::from_chars(str.data(), str.data() + str.size(), *(int64_t*)dst);
                break;
            case SKR_TYPE_CATEGORY_U32:
                std::from_chars(str.data(), str.data() + str.size(), *(uint32_t*)dst);
                break;
            case SKR_TYPE_CATEGORY_U64:
                std::from_chars(str.data(), str.data() + str.size(), *(uint64_t*)dst);
                break;
            case SKR_TYPE_CATEGORY_F32:
                fast_float::from_chars(str.data(), str.data() + str.size(), *(float*)dst);
                break;
            case SKR_TYPE_CATEGORY_F64:
                fast_float::from_chars(str.data(), str.data() + str.size(), *(double*)dst);
                break;
