    ...
    : AsyncService(desc), job_queue(job_queue)
{
    ...
    {
        skr_atomic_store_relaxed(&processing_request_counts[i], 0);
    }
}
...
uint64_t RunnerBase::processing_count(SkrAsyncServicePriority priority) const SKR_NOEXCEPT
{
    ...
    {
        return skr_atomic_load_relaxed(&processing_request_counts[priority]);
    }
    else
    {
        uint64_t count = 0;
        for (uint32_t i = 0 ; i < SKR_ASYNC_SERVICE_PRIORITY_COUNT ; ++i)
        {
            count += skr_atomic_load_relaxed(&processing_request_counts[i]);
        }
        return count;
    }
}
